#include "crsf.h"

#include <string.h>

// Таблица CRC8 для полинома 0xD5. Расчёт по таблице вместо побитового цикла:
// на 400 кбод фреймы идут часто, и восемь итераций на байт складываются
// в заметную нагрузку.
static const uint8_t k_crc8_table[256] = {
    ,
};

uint8_t crsf_crc8(const uint8_t *data, size_t len)
{
    uint8_t crc = 0;
    for (size_t i = 0; i < len; i++) {
        crc = k_crc8_table[crc ^ data[i]];
    }
    return crc;
}

void crsf_parser_init(crsf_parser_t *p, crsf_frame_cb_t cb, void *ctx)
{
    memset(p, 0, sizeof(*p));
    p->cb = cb;
    p->ctx = ctx;
}

void crsf_parser_reset(crsf_parser_t *p)
{
    p->state = CRSF_STATE_WAIT_ADDR;
    p->pos = 0;
    p->expected_len = 0;
}

static void feed_byte(crsf_parser_t *p, uint8_t b);

// Отличает правдоподобный байт адреса от мусора. Строгая проверка по списку
// известных адресов дала бы ложные отбрасывания: в CRSF встречаются адреса
// расширений и сторонних устройств. Отсекаем только заведомо невозможное.
static inline bool plausible_address(uint8_t b)
{
    return b != 0x00;
}

static void feed_byte(crsf_parser_t *p, uint8_t b)
{
    switch (p->state) {
    case CRSF_STATE_WAIT_ADDR:
        if (!plausible_address(b)) {
            p->bytes_discarded++;
            return;
        }
        p->buf[0] = b;
        p->pos = 1;
        p->state = CRSF_STATE_WAIT_LEN;
        return;

    case CRSF_STATE_WAIT_LEN:
        if (b < CRSF_LEN_MIN || b > CRSF_LEN_MAX) {
            // Длина невозможная — значит то, что мы приняли за адрес, им
            // не было. Байт, который сейчас в руках, вполне может оказаться
            // настоящим адресом, поэтому пробуем начать сборку с него.
            p->bytes_discarded++;
            p->pos = 0;
            p->state = CRSF_STATE_WAIT_ADDR;
            feed_byte(p, b);
            return;
        }
        p->buf[1] = b;
        p->expected_len = b;
        p->pos = 2;
        p->state = CRSF_STATE_WAIT_BODY;
        return;

    case CRSF_STATE_WAIT_BODY: {
        p->buf[p->pos++] = b;

        // Полная длина фрейма = len + 2: байты addr и len в len не входят.
        const size_t frame_len = (size_t)p->expected_len + 2;
        if (p->pos < frame_len) {
            return;
        }

        const uint8_t crc_calc = crsf_crc8(&p->buf[2], p->expected_len - 1);
        const uint8_t crc_recv = p->buf[frame_len - 1];

        if (crc_calc == crc_recv) {
            p->frames_ok++;
            if (p->cb) {
                p->cb(p->ctx, p->buf, frame_len);
            }
            crsf_parser_reset(p);
            return;
        }

        // CRC не сошёлся: где-то была рассинхронизация. Просто обнулить
        // буфер нельзя — начало настоящего фрейма может уже лежать внутри.
        // Сдвигаемся на байт и пересобираем остаток заново.
        p->frames_bad_crc++;
        p->bytes_discarded++;

        uint8_t tail[CRSF_FRAME_MAX];
        const size_t tail_len = frame_len - 1;
        memcpy(tail, &p->buf[1], tail_len);

        crsf_parser_reset(p);
        for (size_t i = 0; i < tail_len; i++) {
            feed_byte(p, tail[i]);
        }
        return;
    }
    }
}

void crsf_parser_feed(crsf_parser_t *p, const uint8_t *data, size_t len)
{
    for (size_t i = 0; i < len; i++) {
        feed_byte(p, data[i]);
    }
}
