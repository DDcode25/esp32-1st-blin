...
// Электрический режим линии.
//
// FULL — обычный UART: TX и RX на разных пинах, обмен одновременный.
//
// HALF — один провод на приём и передачу. Так подключается S_PORT пульта
// и однопроводный CRSF: устройства говорят по очереди. Драйвер глушит
// приём на время передачи, иначе плата слышала бы саму себя.
typedef enum {
    LINE_FULL_DUPLEX = 0,
    LINE_HALF_DUPLEX = 1,
} line_mode_t;
...
typedef struct {
    ...
    line_mode_t line;
    // Инверсия сигнала. S-Port от FrSky инвертирован, CRSF — нет.
    // Подключение без учёта этого даёт сплошной мусор на линии.
    bool invert;
} port_pins_t;
