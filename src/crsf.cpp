#include "crsf.h"

namespace {

// Таблица CRC8 для полинома 0xD5. Расчёт по таблице вместо побитового цикла:
// на 400 кбод фреймы идут часто, и восемь итераций на байт складываются в
// заметную нагрузку.
const uint8_t kCrc8Table[256] = {
    ,
};

// Отличает правдоподобный байт адреса от мусора. Строгая проверка по списку
// известных адресов дала бы ложные отбрасывания: в CRSF встречаются адреса
// расширений и сторонних устройств. Поэтому отсекаем только заведомо
// невозможное — нулевой байт.
inline bool plausibleAddress(uint8_t b) {
  return b != 0x00;
}

}  // namespace

uint8_t crsfCrc8(const uint8_t* data, size_t len) {
  uint8_t crc = 0;
  for (size_t i = 0; i < len; i++) {
    crc = kCrc8Table[crc ^ data[i]];
  }
  return crc;
}

void CrsfParser::begin(FrameHandler handler, void* ctx) {
  handler_ = handler;
  ctx_ = ctx;
  reset();
}

void CrsfParser::reset() {
  state_ = State::WAIT_ADDR;
  pos_ = 0;
  expectedLen_ = 0;
}

void CrsfParser::feed(const uint8_t* data, size_t len) {
  for (size_t i = 0; i < len; i++) {
    feedByte(data[i]);
  }
}

void CrsfParser::feedByte(uint8_t b) {
  switch (state_) {
    case State::WAIT_ADDR:
      if (!plausibleAddress(b)) {
        bytesDiscarded_++;
        return;
      }
      buf_[0] = b;
      pos_ = 1;
      state_ = State::WAIT_LEN;
      return;

    case State::WAIT_LEN:
      if (b < CRSF_LEN_MIN || b > CRSF_LEN_MAX) {
        // Длина невозможная — значит то, что мы приняли за адрес, им не было.
        // Байт, который сейчас в руках, вполне может оказаться настоящим
        // адресом, поэтому пробуем начать сборку с него, а не выбрасываем.
        bytesDiscarded_++;
        pos_ = 0;
        state_ = State::WAIT_ADDR;
        feedByte(b);
        return;
      }
      buf_[1] = b;
      expectedLen_ = b;
      pos_ = 2;
      state_ = State::WAIT_BODY;
      return;

    case State::WAIT_BODY: {
      buf_[pos_++] = b;

      // Полная длина фрейма = len + 2 (байты addr и len не входят в len).
      const size_t frameLen = static_cast<size_t>(expectedLen_) + 2;
      if (pos_ < frameLen) {
        return;
      }

      // CRC считается по type + payload, то есть от индекса 2 и до байта
      // перед контрольной суммой.
      const uint8_t crcCalc = crsfCrc8(&buf_[2], expectedLen_ - 1);
      const uint8_t crcRecv = buf_[frameLen - 1];

      if (crcCalc == crcRecv) {
        framesOk_++;
        if (handler_) {
          handler_(ctx_, buf_, frameLen);
        }
        reset();
        return;
      }

      // CRC не сошёлся: где-то была рассинхронизация. Просто обнулить буфер
      // нельзя — начало настоящего фрейма может уже лежать внутри него.
      // Поэтому сдвигаемся на один байт и пересобираем остаток заново.
      framesBadCrc_++;
      bytesDiscarded_++;

      uint8_t tail[CRSF_FRAME_MAX];
      const size_t tailLen = frameLen - 1;
      memcpy(tail, &buf_[1], tailLen);

      reset();
      for (size_t i = 0; i < tailLen; i++) {
        feedByte(tail[i]);
      }
      return;
    }
  }
}
